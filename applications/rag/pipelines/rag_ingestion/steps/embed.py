...
async def embed_action(
    ...
    logger.info(f"🧬 [Embed] Starte Embedding für insgesamt {total_chunks} Chunks von {filename}...")
    ...
    for i in range(0, total_chunks, batch_size):
        ...
        logger.info(f"   📥 Sende Batch {i // batch_size + 1} ({len(batch_texts)} Chunks) an Infinity...")
        ...
        try:
            ...
            logger.error(f"❌ CRASH BEI BATCH-EMBEDDING in Datei {filename}!")
            logger.error(f"Fehlertyp: {type(e).__name__} | Meldung: {str(e)}")
            raise

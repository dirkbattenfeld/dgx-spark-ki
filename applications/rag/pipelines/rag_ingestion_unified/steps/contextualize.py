...
@trace_action(step_name="contextualize")
async def contextualize_action(chunked_doc: ChunkedDocument, vllm_client: Any, config: ContextualizeConfig) -> ChunkedDocument:
    ...
    print(f"🤖 [Contextualize] Starte vLLM für {total} Chunks (max_concurrent={config.max_concurrent}) von {filename}...")
    ...
    async def process_one(chunk: Chunk, idx: int):
        async with sem:
            ...
            try:
                ...
                print(f"💥 vLLM Fehler bei Chunk {idx}: {e}")
                preamble = ""

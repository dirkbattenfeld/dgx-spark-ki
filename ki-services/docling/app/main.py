    ...
    try:
        ...
        state.converter = DocumentConverter(
            ...
            format_options={
                ...
                # Standard-Optionen für Office-Formate (laufen standardmäßig CPU-basiert für Text-Extraktion)
...
async def extract_document(request: ExtractRequest):
    """
    Konvertiert PDF, DOCX, PPTX oder XLSX -> JSON (Struktur) und Markdown.
    ...
    try:
        # Existenzprüfung über SDK (Feldname verallgemeinert auf source_doc)
        if not storage.exists(request.source_doc):
            raise HTTPException(
                status_code=404, 
                detail=f"Datei nicht gefunden: {request.source_doc}"
            )
                    
        # 1. Dateiendung extrahieren und InputFormat bestimmen
        file_extension = Path(request.source_doc).suffix.lower()
        format_mapping = {
            ".pdf": InputFormat.PDF,
        ...
        if not input_format:
            raise HTTPException(
                status_code=400, 
                detail=f"Nicht unterstütztes Dateiformat: {file_extension}. Erlaubt sind PDF, DOCX, PPTX, XLSX."
            )

        # 2. Format-spezifische Optionen konfigurieren
        meta_processed = {
        ...
        if input_format == InputFormat.PDF:
            ...
            logger.info(f"Nutze Standard-Pipeline für Office-Format: {input_format}")
        
        convert_kwargs = {}
            
        # Streamen über SDK
        with storage.open(request.source_doc, mode="rb") as file_stream:
            file_bytes = io.BytesIO(file_stream.read())
            
        file_name = Path(request.source_doc).name
        docling_stream = DocumentStream(name=file_name, stream=file_bytes)
            
        convert_kwargs["source"] = docling_stream
        
        # 3. Seiteneinschränkung nur für PDFs erlauben
        if request.start_page is not None or request.end_page is not None:
            if input_format != InputFormat.PDF:
                raise HTTPException(
                    status_code=400, 
                    detail="Seiteneinschränkungen (page_range) werden nur für PDF-Dokumente unterstützt."
                )
        ...
        # Dokument chronologisch Element für Element durchlaufen
        for element, level in result.document.iterate_items():
            ...
            if element.label == "table" or hasattr(element, "export_to_dataframe"):
                ...
                # 4. Defensives Auslesen der Seitennummer (prov kann bei Office fehlen)
                page_no = None
                ...
                # Wenn verfügbar, Markdown-Variante der Tabelle für das hybride MD nutzen
                if hasattr(element, "export_to_markdown"):
                    table_md = element.export_to_markdown()
                    hybrid_markdown_lines.append(f"\n{table_md}\n")
                else:
                    hybrid_markdown_lines.append(f"")
        ...
        # Saubere Pfade ableiten ohne doppelte Endungen
        base_path, _ = os.path.splitext(request.source_doc)
        ...
        meta_processed["extracted_tables_count"] = table_counter
        meta_processed["pages"] = convert_kwargs.get("page_range", "All")
        ...
        logger.error(f"Kritischer Fehler im Extract-Endpunkt: {e}")
...
async def chunk_document_normalized(request: ChunkRequest):
    ...
    try:
        if not storage.exists(request.json_path):
        ...
        for dl_chunk in base_chunks:
            ...
            # Hier lag der Fehler im Umbau – wir bleiben exakt bei deiner funktionierenden Logik:
            ...
            # Optimierter Token-Check ohne Performance-Verlust
            child_tokens = tokenizer.encode(final_child_text, add_special_tokens=False)
            exact_child_token_count = len(child_tokens)
            ...
            prepared_children.append({
                ...
                "final_child_text": final_child_text, 
                ...
                "exact_token_count": exact_child_token_count 
            })
            global_child_counter += 1
            ...
            for child_idx, child in enumerate(current_window):
                ...
                output_children.append({
                    ...
                    "meta": {
                        ...
                        "source_path": request.source_path,
                        "token_estimate": child["exact_token_count"] 
                    }
                })
